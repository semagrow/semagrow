package org.semagrow.connector.sparql.execution.rxjava;

import org.semagrow.connector.sparql.SPARQLSite;
import org.semagrow.connector.sparql.execution.ConnectionManager;
import org.semagrow.evaluation.rxjava.OnSubscribeTupleResults;
...
public class SPARQLQueryExecutor extends ConnectionManager implements QueryExecutor
{
        ...
        try {
            ...
            if (freeVars.isEmpty()) {
                ...
                result = Observable.just(bindings).flatMap(b -> {
                    try {
                        if (sendBooleanQueryReactive(endpoint.getURL(), sparqlQuery, relevantBindings))
                ...
                result = sendTupleQueryReactive(endpoint.getURL(), sparqlQuery, relevantBindings)
            ...
            throws Exception
    {
        ...
        result = sendTupleQueryReactive(endpoint.getURL(), sparqlQuery, EmptyBindingSet.getInstance());
    ...
    protected Observable<BindingSet>
        sendTupleQueryReactive(URL endpoint, String sparqlQuery, BindingSet bindings)
    ...
    protected boolean
        sendBooleanQueryReactive(URL endpoint, String sparqlQuery, BindingSet bindings)
            throws QueryEvaluationException, MalformedQueryException, RepositoryException {
        ...
        logger.debug("Sending to {} query {} with {}", endpoint.toString(), sparqlQuery.replace('\n', ' '), query.getBindings());
