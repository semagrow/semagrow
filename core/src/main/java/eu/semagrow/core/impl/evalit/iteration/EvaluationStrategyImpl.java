...
public class EvaluationStrategyImpl
    ...
    public CloseableIteration<BindingSet,QueryEvaluationException>
        ...
        if (expr.getSources().size() == 1)
            return evaluateSource(((SPARQLSite)expr.getSources().get(0)).getURI(), innerExpr, bindings);

        for (Site endpoint : expr.getSources()) {
            CloseableIteration<BindingSet,QueryEvaluationException> iter =
                    evaluateSourceAsync(((SPARQLSite)endpoint).getURI(), innerExpr, bindings);
             results.add(iter);
        }
