...
public class QueryExecutorImpl extends ConnectionManager implements QueryExecutor
{
        ...
        try {
            ...
            if (freeVars.isEmpty()) {
                ...
                result = Streams.just(bindings).flatMap(b -> {
                    try {
                        if (sendBooleanQuery(endpoint.getURI(), sparqlQuery, relevantBindings, expr))
                ...
                result = sendTupleQuery(endpoint.getURI(), sparqlQuery, relevantBindings, expr)
            ...
            throws QueryEvaluationException
    {
        ...
        if (!bindings.isEmpty())
            relevant = bindingSetOps.projectNames(exprVars, bindings.get(0));

        try {
            String sparqlQuery = SPARQLQueryStringUtil.buildSPARQLQueryUNION(expr, bindings, relevant);
            result = sendTupleQuery(endpoint.getURI(), sparqlQuery, EmptyBindingSet.getInstance(), expr);
