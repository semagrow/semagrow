...
public class StAsText implements Function {
    
	@Override
    public String getURI() {
        return STRDF.asText.toString();
    }
    ...
    public Value evaluate(ValueFactory valueFactory, Value... values) throws ValueExprEvaluationException {
        ...
        String gmlString = values[0].stringValue();
        String wktString = null;
        try {
        	wktString = SimpleGeometryConverter.getInstance().GMLtoWKT(gmlString);
		} catch (IllegalGeometryException e) {
			throw new ValueExprEvaluationException("Illegal GML format", e);
		}
